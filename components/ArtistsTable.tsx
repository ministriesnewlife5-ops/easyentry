  ...
  const handleAction = async (id: string, action: 'archive' | 'delete') => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error(`Failed to ${action} artist`);
      }
    } catch (error) {
      console.error(`Error ${action}ing artist:`, error);
    } finally {
      setLoading(null);
      ...
      <article className="mt-4 rounded-2xl border border-[#2A2A2A] bg-[#101018] p-5 overflow-x-auto">
        <table className="w-full text-left text-sm text-[#F5F5DC]">
          ...
          <tbody className="divide-y divide-[#2A2A2A]">
              ...
              <tr key={artist.id} className="hover:bg-[#2A2A2A]/40 transition-colors">
                ...
                <td className="px-4 py-4">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => setConfirmAction({ id: artist.id, action: 'delete' })}
                      disabled={loading === artist.id}
            ...
            {data.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-12 text-center text-[#F5F5DC]/50">
                  No artists found
                </td>
              </tr>
            )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="rounded-2xl border border-[#2A2A2A] bg-[#1A1A1A] p-6 max-w-md w-full mx-4">
            ...
            <p className="text-sm text-[#F5F5DC]/70 mb-6">
              Are you sure you want to {confirmAction.action} this artist? 
