  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to approve influencer');
      }
    } catch (error) {
  ...
  const handleAction = async (id: string, action: 'archive' | 'delete') => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error(`Failed to ${action} influencer`);
      }
    } catch (error) {
      console.error(`Error ${action}ing influencer:`, error);
    } finally {
      setLoading(null);
              ...
              <tr key={inf.id} className="hover:bg-[#2A2A2A]/40 transition-colors">
                ...
                <td className="px-4 py-4">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => setConfirmAction({ id: inf.id, action: 'archive' })}
                      disabled={loading === inf.id}
                    ...
                    <button
                      onClick={() => setConfirmAction({ id: inf.id, action: 'delete' })}
                      disabled={loading === inf.id}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="rounded-2xl border border-[#2A2A2A] bg-[#1A1A1A] p-6 max-w-md w-full mx-4">
            ...
            <p className="text-sm text-[#F5F5DC]/70 mb-6">
              Are you sure you want to {confirmAction.action} this influencer?
