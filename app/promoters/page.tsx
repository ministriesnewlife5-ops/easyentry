...
function PromoterCard({ promoter, index }: { promoter: typeof promoters[0]; index: number }) {
  ...
  const shareToWhatsApp = async (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    const url = `${window.location.origin}/promoter/${promoter.id}`;
    window.open(`https://wa.me/?text=${encodeURIComponent(`Check out ${promoter.name}'s profile! ` + url)}`, '_blank');
  };
  ...
  const shareToInstagram = async (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    const url = `${window.location.origin}/promoter/${promoter.id}`;
    navigator.clipboard.writeText(`Check out ${promoter.name}'s profile! ${url}`);
      ...
      >
        ...
        <div className="relative aspect-[3/4] overflow-hidden">
          ...
          {promoter.verified && (
            ...
            >
              <span className="px-3 py-1.5 bg-gradient-to-r from-[#E5A823] to-[#EB4D4B] rounded-full text-xs font-bold text-[#0D0D0D] shadow-lg flex items-center gap-1">
        ...
        <div className="p-5 relative">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-[#F5F5DC]/50">{promoter.companyName}</p>
            </div>
            <motion.div 
              className="text-right"
              animate={{ scale: isHovered ? 1.1 : 1 }}
            >
              <span className="text-xs text-[#E5A823] font-bold">{promoter.eventsPromoted}</span>
              <p className="text-[10px] text-[#F5F5DC]/50 uppercase">events</p>
            </motion.div>
          </div>

          {/* Specialty Tag */}
          <div className="mt-3">
            <span className="inline-flex items-center gap-1 text-xs text-[#F5F5DC]/70">
          ...
          >
            <Link href={`/promoter/${promoter.id}`}>
              <motion.button
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <h1 className="text-4xl md:text-5xl font-black mb-4">
            Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#E5A823] to-[#EB4D4B]">Promoters & Influencers</span>
          </h1>
          <p className="text-[#F5F5DC]/70 text-lg max-w-2xl">
            Connect with top event promoters and influencers. From social media stars to seasoned event professionals, find the perfect partner for your next event.
          </p>
        </motion.div>
